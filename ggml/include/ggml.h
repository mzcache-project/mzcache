...
extern "C" {
    ...
    struct ggml_tensor {
        ...
        // mzcache: points to a struct ggml_tensor_ptrs (mzcache_types.h) — the
        // per-layer table of 256-token KV chunk tensors this tensor belongs to
        void * ggml_tensor_ptrs;
        int64_t is_weight; // mzcache: weight tensors are allocated/unloaded by mzcache
        int64_t padding;   // keep 16-byte struct padding
#else
        char padding[8];
#endif
    };
...
#ifdef MZCACHE_SVM_KV_CHUNK
    // mzcache: ne11 carries the true KV length, which chunked K/V tensors no
    // longer expose through their shape
