...
// mzcache: graph compute waits on per-layer restore progress (layer sync)
extern mzcache_core* g_mzcache_core;

// mzcache: recover the layer index from tensor names like "attn_norm-12"
static int extract_idx(const char* tensor_name, const char* pattern_str) {
    if (!tensor_name || !pattern_str) return -1;
...
#ifdef MZCACHE_SVM_KV_CHUNK
// mzcache: globals consumed by mzcache/src/mzcache_kv_state.cpp (SVM allocation,
// chunk pointer tables) and by the chunked flash-attn path below
cl_context g_opencl_context = nullptr;
cl_command_queue g_opencl_queue = nullptr;
std::vector<chunk_ptrs *> g_svm_chunk_ptrs;
int last_chunk_idx = -1;
static int D = -1; // flash-attn head dim latched in supports_op
#endif // MZCACHE_SVM_KV_CHUNK
...
//mzcache
KernelConfig get_kernel_config(ADRENO_GPU_GEN gen) {
    switch (gen) {
        case ADRENO_GPU_GEN::A8X: // e.g., 830
            return {128, 4};
        case ADRENO_GPU_GEN::A7X: // e.g., 730, 740, 750
            return {64, 2};
        default:
            return {64, 2}; // fallback safe default
    }
}
...
#ifdef MZCACHE_SVM_KV_CHUNK
    cl_program program_mzcache_flash_attn_kvchunk_D64;
    cl_program program_mzcache_flash_attn_kvchunk;
...
#ifdef MZCACHE_SVM_KV_CHUNK
    // mzcache
    cl_kernel kernel_mzcache_flash_attn_f16_kvchunk_D64;
    cl_kernel kernel_mzcache_flash_attn_f16_kvchunk;
...
#ifdef MZCACHE_SVM_KV_CHUNK
    // mzcache : chunked flash-attn — ONE source, constants injected as build
    // options. Two programs per device: D (head dim) is model-dependent, and
    // NWARPS/KQ_TILE come from get_kernel_config so the kernel's compile-time
    // constants always match the host launch config (see invariant #6 in
    // docs/INTEGRATION.md).
    {
#ifdef GGML_OPENCL_EMBED_KERNELS
        const std::string kernel_src {
            #include "mzcache_flash_attn_f16_kvchunk.cl.h"
        };
#else
        const std::string kernel_src = read_file("mzcache_flash_attn_f16_kvchunk.cl");
#endif
        const KernelConfig fa_cfg = get_kernel_config(backend_ctx->adreno_gen);
        const std::string fa_opts = compile_opts +
            " -DNWARPS=" + std::to_string(fa_cfg.NWARPS) +
            " -DKQ_TILE=" + std::to_string(fa_cfg.KQ_TILE);

        backend_ctx->program_mzcache_flash_attn_kvchunk =
            build_program_from_source(backend_ctx->context, backend_ctx->device, kernel_src.c_str(), fa_opts + " -DD=128");
        CL_CHECK((backend_ctx->kernel_mzcache_flash_attn_f16_kvchunk = clCreateKernel(backend_ctx->program_mzcache_flash_attn_kvchunk, "kernel_mzcache_flash_attn_f16_kvchunk", &err), err));
        GGML_LOG_CONT(".");

        backend_ctx->program_mzcache_flash_attn_kvchunk_D64 =
            build_program_from_source(backend_ctx->context, backend_ctx->device, kernel_src.c_str(), fa_opts + " -DD=64");
        CL_CHECK((backend_ctx->kernel_mzcache_flash_attn_f16_kvchunk_D64 = clCreateKernel(backend_ctx->program_mzcache_flash_attn_kvchunk_D64, "kernel_mzcache_flash_attn_f16_kvchunk", &err), err));
        GGML_LOG_CONT(".");
    }
...
#ifdef MZCACHE_SVM_KV_CHUNK
    // mzcache: log work-group limits relevant to the chunked flash-attn kernels
    size_t max_wg = 0;
    clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_GROUP_SIZE,
                    sizeof(max_wg), &max_wg, nullptr);
    MZ_LOG_INFO("  Max work-group size: %zu\n", max_wg);

    size_t maxWISizes[3] = {0,0,0};
    clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_ITEM_SIZES,
                    sizeof(maxWISizes), maxWISizes, nullptr);
    MZ_LOG_INFO("Max WI sizes      : %zu, %zu, %zu\n",
        maxWISizes[0], maxWISizes[1], maxWISizes[2]);

    size_t cu = 0;
    clGetDeviceInfo(device, CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(cu), &cu, nullptr);
    MZ_LOG_INFO("  Compute Units: %zu\n", cu);
#endif
...
#ifdef MZCACHE_SVM_KV_CHUNK
    // mzcache: make context accessible globally
    g_opencl_context = context;
#endif
...
#ifdef MZCACHE_SVM_KV_CHUNK
    g_opencl_queue = backend_ctx->queue;
#endif
...
static ggml_status ggml_backend_opencl_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph) {
    for (int i = 0; i < cgraph->n_nodes; i++) {
        // MZ_LOG_INFO("ggml_opencl: executing node %d/%d: %s (%s)\n", i + 1, cgraph->n_nodes, cgraph->nodes[i]->name, ggml_op_name(cgraph->nodes[i]->op));
        ...
        if (node->op == GGML_OP_RESHAPE || node->op == GGML_OP_TRANSPOSE || node->op == GGML_OP_VIEW || node->op == GGML_OP_PERMUTE || node->op == GGML_OP_NONE) {
            // MZ_LOG_INFO("ggml_opencl: skipping node %s (%s)\n", node->name, ggml_op_name(node->op));
            continue;
        }
...
#ifdef MZCACHE_SVM_KV_CHUNK
        case GGML_OP_FLASH_ATTN_EXT: // implemented for mzcache (chunked KV)
            // ne[1] (query count) <= 256: the kernel tiles queries in the launch
            // grid, but a KV-cache write may not span two 256-token chunks
            // (cpy_k/cpy_v guard), so 256 is the effective ubatch ceiling.
            if ((op->src[0]->ne[0] == 128 || op->src[0]->ne[0] == 64) && op->src[0]->ne[1] <= 256 && op->src[1]->type == GGML_TYPE_F16) {
                D = op->src[0]->ne[0];
                return true;
            }
#endif
...
struct ggml_backend_opencl_buffer_context {
...
#ifdef MZCACHE_SVM_KV_CHUNK
    bool        is_svm        = false;           // true: SVM mode, false: cl_mem mode
    cl_context  clctx         = nullptr;         // for SVM free
    void       *svm_base      = nullptr;         // SVM allocation base pointer

    ggml_backend_opencl_buffer_context(cl_context ctx, void *ptr)
        : is_svm(true), clctx(ctx), svm_base(ptr), name("OpenCL SVM") {}
    ...
    ~ggml_backend_opencl_buffer_context() {
        ...
        if (is_svm) {
            if (svm_base) {
                if (g_opencl_queue) {
                    CL_CHECK(clFinish(g_opencl_queue));
                }
                clSVMFree(clctx, svm_base);
                svm_base = nullptr;
            }

            // MZ_LOG_INFO("ggml_opencl: freeing SVM buffer context %s\n", name.c_str());

            // Delete all extras to trigger their destructors
            for (ggml_tensor_extra_cl * e : temp_tensor_extras) {
                if(e->svm_chunk_ptrs) {
                    clSVMFree(clctx, e->svm_chunk_ptrs);
                    e->svm_chunk_ptrs = nullptr;
                }
                delete e;
            }
...
#ifdef MZCACHE_SVM_KV_CHUNK
    ggml_backend_opencl_buffer_context * ctx = (ggml_backend_opencl_buffer_context *) buffer->context;
    GGML_ASSERT(ctx && "ctx is nullptr?");

    if (ctx->is_svm) {
        // For SVM, the base is the SVM pointer.
        return ctx->svm_base;
    }
#endif
...
static enum ggml_status ggml_backend_opencl_buffer_init_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor) {
    ...
    if(ctx->is_svm) {
        ...
        if(tensor->is_weight) {
            if (tensor->view_src != nullptr) {
                // Unlike KV, weight tensors' data pointer are already set.
                tensor->data = ctx->svm_base;
                ...
                // In fact, it's not a good implementation to set tensor->extra to nullptr here.
                // We should instantiate a new extra for the tensor, which will be used to store offset from svm_base.
                // But for SVM, this implementation is OK, due to shared virtual memory.
                tensor->extra = nullptr; //mzcache original
            }
        }
        else {
            // For KV chunks that are not weights, we set the data pointer to the SVM base.
            tensor->data = ctx->svm_base;

            if (tensor->view_src != nullptr && tensor->view_src->extra) {
                ggml_tensor_extra_cl * view_extra = (ggml_tensor_extra_cl *) tensor->view_src->extra; /// is this how view_src's extra gets carried over?
                tensor->extra = view_extra;
            }
            else {
                tensor->extra = nullptr;
    ...
    if (tensor->view_src != nullptr) {
        ...
        if(tensor->view_src->extra == nullptr){
            GGML_LOG_ERROR("%s: tensor->view_src->extra == nullptr\n", __func__);
    ...
    } else {
        {
            ...
            tensor->data = 0xbb9b080
            ggml_backend_opencl_buffer_get_base(buffer) = 0x80
            offset = 196718592
            ctx->buffer[0] = 0xb400007cc885f1e0
            */
        }
    }
    return GGML_STATUS_SUCCESS;
...
#ifdef MZCACHE_SVM_KV_CHUNK
// Minsung note: to get SVM pointer to copy weights from file directly.
static void* ggml_backend_opencl_get_svm_base_ptr(ggml_backend_buffer_t buffer, size_t offset) {
    ggml_backend_opencl_context *backend_ctx = ggml_cl2_init(buffer->buft->device);
    ggml_backend_opencl_buffer_context * ctx = (ggml_backend_opencl_buffer_context *) buffer->context;
    
    if(ctx->is_svm) {
        return ((char *)ctx->svm_base + offset);
    }else{
        return nullptr;
    }
}
...
static void ggml_backend_opencl_buffer_set_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    ...
    if(ctx->is_svm) {
        clFinish(queue);
        memcpy((char *)ctx->svm_base + offset, data, size);
        return;
    }
#endif
...
static void ggml_backend_opencl_buffer_get_tensor(ggml_backend_buffer_t buffer, const ggml_tensor * tensor, void * data, size_t offset, size_t size) {
    ...
    ggml_backend_opencl_buffer_context * ctx = (ggml_backend_opencl_buffer_context *) buffer->context;
    if(ctx->is_svm) {
        memcpy(data, (char *)ctx->svm_base + tensor->view_offs + offset, size);
        return;
    }
#endif
...
#ifdef MZCACHE_SVM_KV_CHUNK
    if(ctx->is_svm) {
        memset(ctx->svm_base, value, buffer->size);
        return;
    }
#endif
...
#ifdef MZCACHE_SVM_KV_CHUNK
// mzcache
static void ggml_backend_opencl_buffer_set_chunk_offset(ggml_backend_buffer_t buffer, struct ggml_tensor * tensor, int idx) {
    ggml_backend_opencl_buffer_context * ctx = (ggml_backend_opencl_buffer_context *) buffer->context;
    GGML_ASSERT(ctx && "OpenCL buffer context is null");
    GGML_ASSERT(tensor->ggml_tensor_ptrs != NULL && "Tensor is not chunked");
    
    ggml_tensor_extra_cl * extra = ctx->ggml_opencl_alloc_temp_tensor_extra();

    chunk_ptrs *svm_table = static_cast<chunk_ptrs*>(clSVMAlloc(ctx->clctx, CL_MEM_READ_WRITE | CL_MEM_SVM_FINE_GRAIN_BUFFER, sizeof(chunk_ptrs), /*alignment=*/0));
    // printf("ggml_backend_opencl_buffer_set_chunk_offset: svm_table = %p\n", svm_table);

    if(!svm_table) {
        GGML_LOG_ERROR("Failed to allocate SVM table for chunks");
        return;
    }

    // for global access
    g_svm_chunk_ptrs.push_back(svm_table);

    for (int i = 0; i < MAX_CHUNKS_PER_TENSOR; i++) {
        auto *chunk = ((ggml_tensor_ptrs *)tensor->ggml_tensor_ptrs)->c[i];
        if (!chunk) break;
        svm_table->c[i] = chunk->data;  // chunk->data must also be an SVM pointer
    }

    // printf("tensor_name: %s, svm_table->c[0]: %p\n", tensor->name, svm_table->c[0]);

    extra->svm_chunk_ptrs = svm_table;
    extra->clctx = ctx->clctx;
    tensor->extra = extra;

    // MZ_LOG_INFO("Tensor name = %s, idx = %d, svm_chunk_ptrs = %p, tensor->extra = %p\n",
    //             tensor->name, idx, extra->svm_chunk_ptrs, tensor->extra);
}
#endif
...
#ifdef MZCACHE_SVM_KV_CHUNK
    /* .unload_buffers  = */ NULL, // mzcache: OpenCL buffers are unloaded via the multi-buffer
    /* .set_chunk_offset = */ ggml_backend_opencl_buffer_set_chunk_offset,
#endif
};
...
#ifdef MZCACHE_SVM_KV_CHUNK
static ggml_backend_buffer_t ggml_backend_opencl_buffer_type_alloc_svm_buffer(ggml_backend_buffer_type_t buffer_type, size_t size) {
    ggml_backend_opencl_context *backend_ctx = ggml_cl2_init(buffer_type->device);
    // std::cout << "Alloc SVM buffer" << "\n";
    // clCreateBuffer returns -61 for size 0
    size = std::max(size, (size_t)1);

    void *svm_ptr = clSVMAlloc(backend_ctx->context,
                    CL_MEM_READ_WRITE | CL_MEM_SVM_FINE_GRAIN_BUFFER,
                    size, /*alignment*/0);

    // printf("size: %zu, svm_ptr: %p\n", size, svm_ptr);

    if (!svm_ptr) {
        GGML_LOG_INFO("%s: clSVMAlloc failed (%.2f MiB) – falling back\n",
                        __func__, size / 1024.0 / 1024.0);
    }

    ggml_backend_opencl_buffer_context * ctx = new ggml_backend_opencl_buffer_context(backend_ctx->context, svm_ptr);
...
static ggml_backend_buffer_type_i ggml_backend_opencl_buffer_type_interface = {
    ...
    /* .is_host          = */ NULL,
#ifdef MZCACHE_SVM_KV_CHUNK
    /* .alloc_svm_buffer = */ ggml_backend_opencl_buffer_type_alloc_svm_buffer,
#endif
};
...
static void ggml_cl_mul(ggml_backend_t backend, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
...
#ifdef MZCACHE_SVM_KV_CHUNK
    if(strncmp(dst->name, "attn_norm", 9) == 0) {
        if (g_mzcache_core != nullptr && g_mzcache_core->layer_sync_size() > 0) {
            int layer_idx = extract_idx(dst->name, "attn_norm-(\\d+)");
            // std::cout << "Waiting for layer sync for tensor: " << dst->name << " with layer index: " << layer_idx << std::endl;
            if (layer_idx >= 0) {
                // std::cout << "opencl, attn_norm, layer_idx : " << layer_idx << " for tensor: " << dst->name
                //           << " started at: " << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count() % 1000 << " ms" << std::endl;
                // print remaining jobs and sleep
                // if(layer_idx == 15)
                //     g_mzcache_core->wait_polling_layer_sync(layer_idx);

                g_mzcache_core->wait_for_layer_sync(layer_idx);
                // std::cout << "opencl, attn_norm, layer_idx : " << layer_idx << " for tensor: " << dst->name
                //           << " completed at: " << std::chrono::duration_cast<std::chrono::milliseconds>(cur_time.time_since_epoch()).count() % 1000 << " ms" << std::endl;
            } else {
                GGML_LOG_ERROR("%s: Invalid layer index %d for tensor %s\n", __func__, layer_idx, dst->name);
            }
        }
    }
#endif
    ...
    cl_ulong offset1;
    if(src0->is_weight){
        offset0 = src0->view_offs;
        // offset1 = src1->view_offs;
    }else{
        offset0 = extra0->offset + src0->view_offs;
        // offset1 = extra1->offset + src1->view_offs;
    }
    ...
    if (ggml_nelements(src1) == ne10 && ggml_is_contiguous(src1) && ne00 % 4 == 0 && ne10 % 4 == 0) {
        ...
        kernel = backend_ctx->kernel_mul_row;
        if(src0->is_weight){
            CL_CHECK(clSetKernelArgSVMPointer(kernel, 0, src0->data));
        }else{
            CL_CHECK(clSetKernelArg(kernel, 0, sizeof(cl_mem),   &extra0->data_device));
        }
        CL_CHECK(clSetKernelArg(kernel, 1, sizeof(cl_ulong), &offset0));
        if(src1->is_weight){
            CL_CHECK(clSetKernelArgSVMPointer(kernel, 2, src1->data));
        }else{
            CL_CHECK(clSetKernelArg(kernel, 2, sizeof(cl_mem),   &extra1->data_device));
        }
        CL_CHECK(clSetKernelArg(kernel, 3, sizeof(cl_ulong), &offset1));
...
static void ggml_cl_mul_mat(ggml_backend_t backend, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
...
#ifdef MZCACHE_SVM_KV_CHUNK
    //For Exaone4
    if(strncmp(src1->name, "OpenCL#", 7) == 0) {
        if (g_mzcache_core != nullptr && g_mzcache_core->layer_sync_size() > 0) {
            int layer_idx = 0;
            g_mzcache_core->wait_for_layer_sync(layer_idx);
    ...
    else if(strncmp(src1->name, "l_out-", 6) == 0) {
        if (g_mzcache_core != nullptr && g_mzcache_core->layer_sync_size() > 0) {
            int layer_idx = extract_idx(src1->name, "out-(\\d+)") + 1;
            g_mzcache_core->wait_for_layer_sync(layer_idx);
            auto cur_time = std::chrono::high_resolution_clock::now();
            // std::cout << "opencl, attn_norm, layer_idx : " << layer_idx << " for tensor: " << dst->name
            //             << " completed at: " << std::chrono::duration_cast<std::chrono::milliseconds>(cur_time.time_since_epoch()).count() % 1000 << " ms" << std::endl;
...
static void ggml_cl_cpy(ggml_backend_t backend, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    ggml_tensor_extra_cl * extra1;
    cl_ulong offset1;

    void * src1_data;

    if(src1->name && strncmp(src1->name, "cache", 5) == 0) {
        // MZ_LOG_INFO("ggml_cl_cpy start\n");
        offset1 = src1->view_offs;

        int layer_idx = extract_idx(src1->name, "_l(\\d+)");
        // std::cout << "src1->name: " << src1->name << ", layer_idx: " << layer_idx << std::endl;

        if(last_chunk_idx < 0) {
            MZ_LOG_ERROR("ggml_cl_cpy: last_chunk_idx is negative, this should not happen");
        }

        if(strncmp(src1->name, "cache_k", 7) == 0) {
            // src1 is SVM chunk
            src1_data = g_svm_chunk_ptrs[2*layer_idx]->c[last_chunk_idx];
        } else if(strncmp(src1->name, "cache_v", 7) == 0) {
            // src1 is SVM chunk
            src1_data = g_svm_chunk_ptrs[2*layer_idx + 1]->c[last_chunk_idx];
        } else {
            GGML_ASSERT(false && "Unknown cache chunk name");
        }

        CL_CHECK(clSetKernelArg(kernel, 0, sizeof(cl_mem),   &extra0->data_device));
        CL_CHECK(clSetKernelArg(kernel, 1, sizeof(cl_ulong), &offset0));

        CL_CHECK(clSetKernelArgSVMPointer(kernel, 2, src1_data));
        CL_CHECK(clSetKernelArg(kernel, 3, sizeof(cl_ulong), &offset1));
    }
    else {
        extra1 = (ggml_tensor_extra_cl *)src1->extra;
        offset1 = extra1->offset + src1->view_offs;
    ...
    // MZ_LOG_INFO("ne00 %d, ne01 %d, ne02 %d, ne03 %d, ne10 %d, ne11 %d, ne12 %d, ne13 %d\n",
...
#ifdef MZCACHE_SVM_KV_CHUNK
    // MZ_DEBUG_CPY=1: after each cache-chunk copy, force completion and read
    // the destination back from the CPU (diagnosing lost SVM writes).
    {
        static const bool dbg_cpy = [] {
            const char * e = getenv("MZ_DEBUG_CPY");
            return e && e[0] == '1';
        }();
        if (dbg_cpy && src1->name && strncmp(src1->name, "cache", 5) == 0) {
            CL_CHECK(clFinish(queue));
            const __fp16 * p = (const __fp16 *)((const uint8_t *) g_svm_chunk_ptrs
                [2 * extract_idx(src1->name, "_l(\\d+)") + (strncmp(src1->name, "cache_v", 7) == 0 ? 1 : 0)]
                ->c[last_chunk_idx] + src1->view_offs);
            printf("MZ_DEBUG_CPY %s chunk=%d offs=%zu vals=%.4f %.4f %.4f %.4f\n",
                   src1->name, last_chunk_idx, (size_t) src1->view_offs,
    ...
    // MZ_LOG_INFO("ggml_cl_cpy: src0 %s, src1 %s, dst %s, ne00 %d, ne01 %d, ne02 %d, ne03 %d\n",
...
#ifdef MZCACHE_SVM_KV_CHUNK
// mzcache: flash-attn over chunked SVM-backed KV
static void ggml_cl_mzcache_flash_attn(ggml_backend_t backend, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst)
{

    // MZ_LOG_INFO("ggml_cl_mzcache_flash_attn: src0=%p, src1=%p, dst=%p", (void *)src0, (void *)src1, (void *)dst);
    // MZ_LOG_INFO("src1->name: %s", src1 ? src1->name : "null");
    ...
    int NCOLS = 8;
    int layer_idx = -1;  // default value

    // Check that src0->name exists and is long enough

    // std::cout << "ggml_cl_mzcache_flash_attn: layer_idx = " << layer_idx << std::endl;
    // int KQ_stride = -1;

    // int KQ_granularity = 64;
    
    const ggml_tensor * Q    = src0;
    const ggml_tensor * K    = src1;
    ...
    // The kernel tiles queries over work-group x (NCOLS per group); 256 is the
    // system-wide ubatch ceiling (a KV-cache write may not span two chunks,
    // see cpy_k/cpy_v), mirrored by supports_op.
    GGML_ASSERT(Q->ne[1] <= 256); // input sequence length
    GGML_ASSERT(K->ne[1] == V->ne[1]);

    float scale         = 1.0f;   // set later from dst->op_params[0]
    float max_bias      = 0.0f;   // dst->op_params[1]
    float logit_softcap = 0.0f;   // fixed

    memcpy(&scale,         (const float *) dst->op_params + 0, sizeof(float));

    // 4) OpenCL context, queue, kernel
    ggml_backend_opencl_context *backend_ctx = (ggml_backend_opencl_context *)backend->context;
    cl_command_queue queue = backend_ctx->queue;

    // 5) buffer offsets
    ggml_tensor_extra_cl * eQ = (ggml_tensor_extra_cl *) Q->extra;
    ggml_tensor_extra_cl * eM = (ggml_tensor_extra_cl *) mask->extra;
    ggml_tensor_extra_cl * eKQV = (ggml_tensor_extra_cl *) dst->extra;

    const cl_ulong offQ = eQ->offset + Q->view_offs;
    const cl_ulong offM = eM->offset + mask->view_offs;
    const cl_ulong offKQV = eKQV->offset + dst->view_offs;

    const cl_ulong nb11 = K->nb[1];
    const cl_ulong nb12 = K->nb[2];
    const cl_ulong nb13 = K->nb[3]; // not used

    const cl_ulong nb21 = V->nb[1];
    const cl_ulong nb22 = V->nb[2];
    const cl_ulong nb23 = V->nb[3]; // not used

    int arg = 0;

    layer_idx = extract_idx(Q->name, "Qcur-(\\d+)");

    chunk_ptrs * k_svm_chunk_ptrs = (chunk_ptrs *)g_svm_chunk_ptrs[2 * layer_idx + 0];
    chunk_ptrs * v_svm_chunk_ptrs = (chunk_ptrs *)g_svm_chunk_ptrs[2 * layer_idx + 1];

    float n_kv_f = -1.0f;
    memcpy(&n_kv_f, (const float *) dst->op_params + 3, sizeof(float));

    // printf("ggml_cl_mzcache_flash_attn: n_kv_f = %f\n", n_kv_f);

    int n_kv = (int)n_kv_f;

    int chunk_num = n_kv / TOKENS_PER_CHUNK;
    if (n_kv % TOKENS_PER_CHUNK != 0) {
        MZ_LOG_ERROR("ggml_cl_mzcache_flash_attn: n_kv is not a multiple of TOKENS_PER_CHUNK");
    }
    cl_kernel kernel;

    KernelConfig cfg = get_kernel_config(backend_ctx->adreno_gen);

    int KQ_TILE = cfg.KQ_TILE;
    int NWARPS  = cfg.NWARPS;

    // NWARPS/KQ_TILE are baked into the program at build time from the same
    // get_kernel_config values used for the launch config below, so the two
    // can never diverge; only the head dim selects the program here.
    if(D == 64)
        kernel = backend_ctx->kernel_mzcache_flash_attn_f16_kvchunk_D64;
    else if(D == 128)
        kernel = backend_ctx->kernel_mzcache_flash_attn_f16_kvchunk;
    else
        GGML_ASSERT(false);

    //determine chunk count and precompute offsets; test once the kernel is implemented
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_mem),   &eQ->data_device));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_ulong), &offQ));
    clSetKernelArgSVMPointer(kernel, arg++, k_svm_chunk_ptrs);
    clSetKernelArgSVMPointer(kernel, arg++, v_svm_chunk_ptrs);

    size_t local_kq_bytes = NCOLS * KQ_TILE * sizeof(uint16_t);
    CL_CHECK(clSetKernelArg(kernel, arg++, local_kq_bytes, NULL));

    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_mem),   &eM->data_device));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_ulong), &offM));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_mem),   &eKQV->data_device));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_ulong), &offKQV));

    
    const uint32_t n_head_log2 = 0;
    const float m0 = 0.0f; // powf(2.0f, -(max_bias       ) / n_head_log2);
    const float m1 = 0.0f; // powf(2.0f, -(max_bias / 2.0f) / n_head_log2);
    ...
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(uint32_t), &n_head_log2));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(float),    &logit_softcap));

    // — shape/stride args
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(int),      &(Q->ne[0])));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(int),      &(Q->ne[1])));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(int),      &(Q->ne[2])));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(int),      &(Q->ne[3])));

    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(int),      &(K->ne[0])));

    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(int),      &(n_kv)));


    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(int),      &(K->ne[2])));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(int),      &(K->ne[3])));

    //mask
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(int),      &(mask->ne[1])));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_ulong), &(mask->nb[1])));

    //Q->nb
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_ulong), &(Q->nb[1])));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_ulong), &(Q->nb[2])));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_ulong), &(Q->nb[3])));
    ...
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_ulong), &nb21));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_ulong), &nb22));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_ulong), &nb23));

    //dst->ne
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(int),      &(dst->ne[0])));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(int),      &(dst->ne[1])));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(int),      &(dst->ne[2])));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(int),      &(dst->ne[3])));

    const int ntiles_x = ((Q->ne[1] + NCOLS - 1) / NCOLS); // one work-group per NCOLS-query tile
    // const int ntiles_total = ntiles_x * (Q->ne[2] / NCOLS) *8Q->ne[3];

    // size_t sg  = 0;
    // clGetDeviceInfo(ctx->device, CL_DEVICE_MAX_SUB_GROUP_SIZE_FOR_NDRANGE, sizeof(sg), &sg, NULL);

    // printf("sub-group size = %zu\n", sg);
    // 7) determine work-group size
    const int  WARP_SZ = 64;
    size_t local_ws[3]  = { (size_t)WARP_SZ, (size_t)NWARPS, 1 };


    const int blocks_x = ntiles_x;
    const int blocks_y = 1; // parallel_blocks=1
    const int blocks_z = Q->ne[2] * Q->ne[3]; // 16 * 1 = 16

    size_t global_ws[3] = {
        (size_t)(blocks_x * WARP_SZ), // input_tokens / NCOLS * WARP_SZ
        (size_t)(blocks_y * NWARPS),  // parallel_blocks * NWARPS
        (size_t)(blocks_z)             // Q->ne[2] * Q->ne[3]
    };

    // printf("ggml_cl_mzcache_flash_attn: global_ws = %zu, %zu, %zu\n", global_ws[0], global_ws[1], global_ws[2]);
    // qwen3_0.6B : 16 groups, llama3_1B : 32 groups.
    

#ifdef GGML_OPENCL_PROFILING
        cl_event evt;
        CL_CHECK(clEnqueueNDRangeKernel(queue, kernel, 3, NULL, global_ws, local_ws, 0, NULL, &evt));

        g_profiling_info.emplace_back();
        populateProfilingInfo(g_profiling_info.back(), evt, kernel, global_ws, local_ws, dst);
#else
    CL_CHECK(clEnqueueNDRangeKernel(queue, kernel, 3, NULL, global_ws, local_ws, 0, NULL, NULL));
#endif
...
bool ggml_cl_compute_forward(ggml_backend_t backend, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_FLASH_ATTN_EXT:
            ...
            func = ggml_cl_mzcache_flash_attn;
            break;
#endif
        default:
            return false;
