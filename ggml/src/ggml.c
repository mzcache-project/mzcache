...
static struct ggml_tensor * ggml_new_tensor_impl(
    ...
    // mzcache: views inherit the chunk-pointer table and weight flag of their source
    void * ggml_tensor_ptrs = view_src != NULL ? view_src->ggml_tensor_ptrs : NULL;
    int this_weight = 0;
    if(view_src != NULL && view_src->is_weight){
        this_weight = 1;
    }
    *result = (struct ggml_tensor) {
        ...
        /*.name         =*/ { 0 },
        /*.extra        =*/ NULL,
        /*.ggml_tensor_ptrs=*/ ggml_tensor_ptrs,
        /*.is_weight    =*/ this_weight,
        /*.padding      =*/ 0,
    };
#else
    *result = (struct ggml_tensor) {
...
#ifdef MZCACHE_SVM_KV_CHUNK
    // mzcache: the true KV length (ne11) rides along in op_params because the
    // chunked K/V tensors no longer expose it via their shape
    float params[] = { scale, max_bias, logit_softcap, (float)ne11 };
#else
...
#ifdef MZCACHE_SVM_KV_CHUNK
    ggml_set_op_params_i32(a, 4, prec_i32); // scale is on first pos, max_bias on second
#else
...
#ifdef MZCACHE_SVM_KV_CHUNK
    const int32_t prec_i32 = ggml_get_op_params_i32(a, 4);
#else
