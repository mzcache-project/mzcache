...
int main(int argc, char** argv) {
    ...
    // Space savings of the KV compression backend (CacheGen / FLEXGEN-8bit),
    // reported over the chunks compressed this swapout: saved_bytes / raw_bytes.
    // (v0.4 mz_prefill_custom logged compression_rate = compressed/raw;
    //  space_savings = 1 - compression_rate.) FLEXGEN-8bit is a fixed 17/32
    // footprint -> ~46.9% regardless of model; CacheGen is data-dependent.
    if (n_comp_chunks > 0) {
        const int64_t chunk_bytes =
            (int64_t) TOKENS_PER_CHUNK * kv_state_ptr->kv_hidden_dim * sizeof(__fp16) * 2;
        const double space_savings_pct =
            100.0 * (double) kv_state_ptr->n_cur_comp_saved_bytes /
            ((double) n_comp_chunks * chunk_bytes);
        std::cout << "[SpaceSavings] comp_chunks=" << n_comp_chunks
                  << " saved_bytes=" << kv_state_ptr->n_cur_comp_saved_bytes
                  << " space_savings_pct=" << space_savings_pct << "\n";
    }
    ...
    std::cout << "\n Press Enter to start prefill..." << std::endl;
    std::cin.get();
