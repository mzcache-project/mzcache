...
#define KQ_TILE    128                /* 256 is slower, 128 is faster for Adreno */
...
#endif
__kernel void kernel_mzcache_flash_attn_f16_kvchunk_D64(
    ...
    const int ne0,  const int ne1,  const int ne2,  const int ne3)     /* dst */
{
    ...
    const int sK  = nb11 / sizeof(half2);    /* 2048 / sizeof(half2) = 32    */
        ...
        for(int k_VKQ_0 = 0; k_VKQ_0 < TOKENS_PER_CHUNK; k_VKQ_0 += KQ_TILE) {
            ...
            for(int k_KQ = 0; k_KQ < D/2; ++k_KQ) {
               ...
               for (int j0 = 0; j0 < NCOLS; j0 += NWARPS) {
                    ...
                    ? Qf2[j*sQ1 + k_KQ]
                    : (float2)(0, 0);
                    Q_k[j0/NWARPS] = (half2)(v.x * scale, v.y * scale);
                }
                for(int i_KQ_0 = 0; i_KQ_0 < KQ_TILE; i_KQ_0 += WARP){
