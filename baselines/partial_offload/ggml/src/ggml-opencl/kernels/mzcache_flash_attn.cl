#pragma OPENCL EXTENSION cl_khr_fp16 : enable
#pragma OPENCL EXTENSION cl_khr_subgroups : enable

#if defined(cl_qcom_reqd_sub_group_size)
#pragma OPENCL EXTENSION cl_qcom_reqd_sub_group_size : enable
#define ADRENO_GPU 1
#define REQD_SUBGROUP_SIZE_64  __attribute__((qcom_reqd_sub_group_size("half")))
#define REQD_SUBGROUP_SIZE_128 __attribute__((qcom_reqd_sub_group_size("full")))
#endif

#define N_F16_F16 4

/*--------------------------------- hyper-params --------------------------------*/
#define WARP      64                /* warp size, 64 threads per warp              */
#define NWARPS     4                 /* work-group y = 4 warps               */
#define WG_SIZE  (WARP*NWARPS)       /* 128 threads                          */

#define D         128                /* head-dim, compile-time constant      */
#define NCOLS      16                /* query cols per tile                  */
#define KQ_TILE    64                /* keys processed per iteration         */

#define HALF_MAX_HALF ((half)32752.0h) 
/*--------------------------------- helpers ------------------------------------*/
inline half hmax2(half a, half b) { return a > b ? a : b; }

inline float get_alibi_slope(
    const float max_bias,
    const uint h,
    const uint n_head_log2,
    const float m0,
    const float m1
) {
    if (max_bias <= 0.0f) {
        return 1.0f;
    }

    float base = (h < n_head_log2) ? m0 : m1;
    int   exph = (h < n_head_log2) ? (int)(h + 1) : (int)(2 * (h - n_head_log2) + 1);

    return pow(base, (float)exph);
}


/*--------------------------------- kernel -------------------------------------*/
#ifdef ADRENO_GPU
REQD_SUBGROUP_SIZE_64
#endif
__kernel void kernel_mzcache_flash_attn(
    /* base pointers + offsets --------------------------------------------------*/
    __global char  *Q,    ulong offQ,
    __global char  *K,    ulong offK,
    __global char  *V,    ulong offV,
    __global char  *mask, ulong offM,
    __global char  *dst,  ulong offDst,

    /* op params --------------------------------------------------------------- */
    const float scale,
    const float max_bias,  const float m0,   const float m1,      /* unused   */
    const uint  n_head_log2,               const float logit_softcap, /* =1    */

    /* shapes / strides -------------------------------------------------------- */
    const int ne00, const int ne01, const int ne02, const int ne03,   /* Q */
    const int ne10, const int ne11, const int ne12, const int ne13,   /* K/V */
    const int ne31, const ulong nb31,
    const ulong nb01, const ulong nb02, const ulong nb03,             /* Q nb */
    const ulong nb11, const ulong nb12, const ulong nb13,             /* K nb */
    const ulong nb21, const ulong nb22, const ulong nb23,             /* V nb */
    const int ne0,  const int ne1,  const int ne2,  const int ne3)     /* dst */
{
    /*---------------------- apply base-offsets --------------------------------*/
    Q = (global char *)((global char *)Q + offQ);
    K = (global char *)((global char *)K + offK);
    V = (global char *)((global char *)V + offV);
    mask = (global char *)((global char *)mask + offM);
    dst = (global char *)((global char *)dst + offDst);

    // float one = 1.0f;
    // float sub_group_reduce = sub_group_reduce_add(one);
    // float work_group_reduce = work_group_reduce_add(one);

    // if (get_global_id(0) == 0) {
    //     uint sg_size = get_sub_group_size();
    //     uint local_size_0 = get_local_size(0);
    //     uint local_size_1 = get_local_size(1);
    //     uint local_size_2 = get_local_size(2);

    //     uint global_size_0 = get_global_size(0);
    //     uint global_size_1 = get_global_size(1);
    //     uint global_size = get_global_size(2);
        
    //     printf("Sub-group size = %u\n", sg_size);
    //     printf("Local size = %u x %u x %u\n", local_size_0, local_size_1, local_size_2);
    //     printf("Global size = %u x %u x %u\n", global_size_0, global_size_1, global_size);

        
    //     printf("Sub-group reduce add = %f\n", sub_group_reduce);

        
    //     printf("Work-group reduce add = %f\n", work_group_reduce);
    // }


    /*---------------------- thread / group indices ----------------------------*/
    const int lane = get_local_id(0);                /* 0‥63               */
    const int warp = get_local_id(1);                /* 0‥3                */
    const int head = get_group_id(2);                /* 0‥ne02-1 (16)      */

    const int ic0  = 0;    /* this kernel is built for the single-tile (nq≤16) */

    /*---------------------- element strides (in elements) ---------------------*/
    const int sQ1 = nb01 / sizeof(float2);   /* stride in q-rows     (1024)   */
    const int sQ2 = nb02 / sizeof(float2);   /* stride per head      (  64)   */
    const int sKV = nb11 / sizeof(half2);    /* stride in k/v rows   ( 512)   */
    const int sM  = nb31 / sizeof(half);    /* stride between q rows (nkv)   */

    // printf("sM : %d\n", sM);
    // printf("ne11 : %d\n", ne11);

    /* grouped-query attention pointer adjust */
    const int gqa_ratio = ne02 / ne12;       /* 16 / 8 = 2 */

    global float2 *Qf2 = (global float2 *)(Q + nb02 * head);
    global half2  *Kh2 = (global half2 *)(K + nb12 * (head / gqa_ratio));
    global half2  *Vh2 = (global half2 *)(V + nb12 * (head / gqa_ratio));
    global const half *Mh = (global const half *)mask + ne11*ic0;
    global float *dst_f = (global float *)dst;


    const float slopef = get_alibi_slope(max_bias, head, n_head_log2, m0, m1);


    /*---------------------- shared buffers ------------------------------------*/
    __local float2 Ql[NCOLS][D/2];           /* 4 KB  */
    __local half2 KV[KQ_TILE][D/2+1];       /* 16.6 KB */
    __local float  KQ[NCOLS*KQ_TILE];        /* 2 KB  */
    // __local half  red[WG_SIZE];             /* 256 B */

    /* ---- per-column accumulators (one “register file” per warp group) ---- */
    float  kqmax[NCOLS/NWARPS];
    float  kqsum[NCOLS/NWARPS];
    float2 VKQ [NCOLS/NWARPS][D/2/WARP]; // (16/4 , 64/32) = (4, 2) registers per column

    /* ── thread-cooperative zeroing & min-init ───────────────────────────── */
    for (int i = 0; i < NCOLS / NWARPS; i += WARP) {
        kqmax[i] = -FLT_MAX/2.0f;              // == CUDA initial value
        kqsum[i] = 0.0f;                  // == CUDA initial value

        /* VKQ has two half2 registers per column (D/2 == 64, 64/WARP == 2) */
        for (int j = 0; j < D / 2 / WARP; ++j)
            VKQ[i][j] = (float2)(0.0f, 0.0f);
    }

    barrier(CLK_LOCAL_MEM_FENCE);

    /*---------------------- 1. load & scale Q ---------------------------------*/
    for(int j=warp; j<NCOLS; j+=NWARPS)
        for(int i=lane; i<D/2; i+=WARP){
            float2 v = (ic0+j < ne01) ? Qf2[j*sQ1 + i] : (float2)(0,0);
            Ql[j][i] = (v * scale); // scale Q
        }
    barrier(CLK_LOCAL_MEM_FENCE);

    /*====================== 2. iterate over K chunks ==========================*/
    // 2. iterate over K chunks
    for(int k_VKQ_0 = 0; k_VKQ_0 < ne11; k_VKQ_0 += KQ_TILE) {
        //
        // 2-A. init kqmax_new = previous kqmax
        //
        float kqmax_new[NCOLS/NWARPS]; // size 4
        for(int jg = 0; jg < NCOLS/NWARPS; ++jg){
            kqmax_new[jg] = kqmax[jg];
        }

        //
        // 2-B. load K chunk into __local[KQ_TILE][D/2+1]
        //
        for(int i_KQ = warp; i_KQ < KQ_TILE; i_KQ += NWARPS){
            for(int k_KQ = lane; k_KQ < D/2; k_KQ += WARP){
                KV[i_KQ][k_KQ] = Kh2[(k_VKQ_0 + i_KQ)*sKV + k_KQ];
            }
        }
        barrier(CLK_LOCAL_MEM_FENCE);

        float2 sum2[KQ_TILE/WARP][NCOLS/NWARPS]; // 1 x 4
        for(int i = 0; i < KQ_TILE/WARP; ++i){
            for(int jg = 0; jg < NCOLS/NWARPS; ++jg){
                sum2[i][jg] = (float2)(0.0f, 0.0f);
            }
        }

        //
        // 2-C. compute logits = Q·K + mask*slope
        for(int k_KQ = 0; k_KQ < D/2; ++k_KQ) {
            half2 K_k[KQ_TILE/WARP]; // 1 element
            float2 Q_k[NCOLS/NWARPS]; // 4 element

            for(int i_KQ_0 = 0; i_KQ_0 < KQ_TILE; i_KQ_0 += WARP){
                const int i_KQ = i_KQ_0 + lane;
                K_k[i_KQ_0/WARP] = KV[i_KQ][k_KQ]; // K_k is private
            }

            for(int j_KQ_0 = 0; j_KQ_0 < NCOLS; j_KQ_0 += NWARPS){
                const int j_KQ = j_KQ_0 + warp;
                Q_k[j_KQ_0/NWARPS] = Ql[j_KQ][k_KQ];
            }

            for(int i_KQ_0 = 0; i_KQ_0 < KQ_TILE; i_KQ_0 += WARP){
                for(int j_KQ_0 = 0; j_KQ_0 < NCOLS; j_KQ_0 += NWARPS){
                    // sum2[i_KQ_0/WARP][j_KQ_0/NWARPS] +=
                    //     (half2)(K_k[i_KQ_0/WARP].x * Q_k[j_KQ_0/NWARPS].x,
                    //             K_k[i_KQ_0/WARP].y * Q_k[j_KQ_0/NWARPS].y);
                    sum2[i_KQ_0/WARP][j_KQ_0/NWARPS] +=
                        convert_float2(K_k[i_KQ_0/WARP]) * Q_k[j_KQ_0/NWARPS]; // odd, even indices are accumulated separately
                }
            }
        }

        for(int i_KQ_0 = 0; i_KQ_0 < KQ_TILE; i_KQ_0 += WARP){
            const int i_KQ = i_KQ_0 + lane;
            for(int j_KQ_0 = 0; j_KQ_0 < NCOLS; j_KQ_0 += NWARPS){
                const int j_KQ = j_KQ_0 + warp;
                float sum = sum2[i_KQ_0/WARP][j_KQ_0/NWARPS].x + sum2[i_KQ_0/WARP][j_KQ_0/NWARPS].y;
                // apply mask & slope
                if(Mh) {
                    half m_val = Mh[j_KQ*ne11 + (k_VKQ_0 + i_KQ)];
                    sum += (slopef * (float)m_val);
                }
                // update kqmax_new
                kqmax_new[j_KQ_0/NWARPS] = fmax(kqmax_new[j_KQ_0/NWARPS], sum);

                KQ[j_KQ*KQ_TILE + i_KQ] = sum;
            }
        }

        barrier(CLK_LOCAL_MEM_FENCE);
        
        
        
        //
        // 2-D. re-scale previous accumulators to new max, then softmax & V-accum
        //
        for(int j0 = 0; j0 < NCOLS; j0 += NWARPS) {
            int j = j0 + warp;

            // warp_reduce_max
            kqmax_new[j0 / NWARPS] = sub_group_reduce_max(kqmax_new[j0 / NWARPS]);

            // only lane 0 needs to compute the scale factor
            float old_max = kqmax[j0/NWARPS];
            float new_max = kqmax_new[j0/NWARPS];
            // exp(old_max - new_max) packed into half2
            float max_diff = old_max - new_max;
            float KQ_max_scale = exp(max_diff);
            // update kqmax
            kqmax[j0/NWARPS] = new_max;
            // re-scale previous rowsum

            float kqsum_add = 0.0f;
            for(int i = lane; i < KQ_TILE; i += WARP){

                const float diff = KQ[j*KQ_TILE + i] - new_max;
                const float val = exp(diff);
                
                kqsum_add += val;                
                KQ[j*KQ_TILE + i] = val;
            }
            kqsum[j0/NWARPS] = kqsum[j0/NWARPS] * KQ_max_scale + kqsum_add;

            for(int i0 = 0; i0 < D/2; i0 += WARP){
                float2 v = VKQ[j0/NWARPS][i0/WARP];
                // re-scale previous accumulators
                v.x = v.x * KQ_max_scale;
                v.y = v.y * KQ_max_scale;
                // reset VKQ[j0/NWARPS][i0/WARP] to zero
                VKQ[j0/NWARPS][i0/WARP] = v;
            }

            
        }
        barrier(CLK_LOCAL_MEM_FENCE);

        for(int k = warp; k < KQ_TILE; k += NWARPS){
            for(int i = lane; i < D/2; i += WARP){
                KV[k][i] = Vh2[(k_VKQ_0 + k)*sKV + i];
            }
            
        }
        barrier(CLK_LOCAL_MEM_FENCE);

        // 2-F. accumulate V with softmax weights
        for(int k0 = 0; k0 < KQ_TILE; k0 += 2) {
            half2 V_k[(D/2)/WARP][2];
            float2 KQ_k[NCOLS/NWARPS];

            for(int i0 = 0; i0 < D/2; i0 += WARP){
                const int i = i0 + lane;

                V_k[i0/WARP][0] = KV[k0][i];
                V_k[i0/WARP][1] = KV[k0+1][i];
            }

            for(int j0 = 0; j0 < NCOLS; j0 += NWARPS){
                const int j = j0 + warp;

                KQ_k[j0/NWARPS] = (float2)(KQ[j*KQ_TILE + k0], KQ[j*KQ_TILE + k0 + 1]);
            }

            for(int i0 = 0; i0 < D/2; i0 += WARP){
                for(int j0 = 0; j0 < NCOLS; j0 += NWARPS){
                    const int i = i0 + lane;
                    const int j = j0 + warp;

                    // accumulate V with softmax weights
                    float2  V0 = convert_float2( V_k[i0/WARP][0] );
                    float2  V1 = convert_float2( V_k[i0/WARP][1] );
                    VKQ[j0/NWARPS][i0/WARP] += V0 * KQ_k[j0/NWARPS].x;
                    VKQ[j0/NWARPS][i0/WARP] += V1 * KQ_k[j0/NWARPS].y;
                }
            }

        }
        barrier(CLK_LOCAL_MEM_FENCE);
    } // end K-chunk loop

    /*====================== 3. write results ==================================*/
    for(int j_VKQ_0 = 0; j_VKQ_0 < NCOLS; j_VKQ_0 += NWARPS) {
        const int j_VKQ = j_VKQ_0 + warp;

        if(ic0 + j_VKQ >= ne01) {
            // if this column is out of bounds, skip it
            return;
        }

        float kqsum_j = sub_group_reduce_add(kqsum[j_VKQ_0/NWARPS]);

        for(int i00 = 0; i00 < D; i00 += 2*WARP) {
            const int i0 = i00 + 2*lane;

            if(i0 < D) {
                float2 dst_val = VKQ[j_VKQ_0/NWARPS][i0/(2*WARP)];

                // normalize by sum
                dst_val.x /= kqsum_j;
                dst_val.y /= kqsum_j;
                // write to global memory
                const int j_dst = j_VKQ;
                dst_f[j_dst * D * ne02 + D * head + i0] = (float)dst_val.x;
                dst_f[j_dst * D * ne02 + D * head + i0 + 1] = (float)dst_val.y;

            }
        }
    }
}
