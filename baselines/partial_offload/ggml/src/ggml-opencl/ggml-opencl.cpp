...
struct ggml_backend_opencl_context {
    ...
    cl_program program_mzcache_flash_attn_nolocalKV_D64;
    cl_program program_mzcache_flash_attn_nolocalKV;
    ...
    cl_kernel kernel_mzcache_flash_attn_f16_nolocalKV_D64;
    cl_kernel kernel_mzcache_flash_attn_f16_nolocalKV;
...
static void load_cl_kernels(ggml_backend_opencl_context *backend_ctx, ggml_cl_version opencl_c_version) {
    ...
    // mzcache : flash_attn_f16_kvchunk_D64
    {
#ifdef GGML_OPENCL_EMBED_KERNELS
        const std::string kernel_src {
            #include "mzcache_flash_attn_f16_kvchunk_D64.cl.h"
        };
#else
        const std::string kernel_src = read_file("mzcache_flash_attn_f16_kvchunk_D64.cl");
#endif
        backend_ctx->program_mzcache_flash_attn_kvchunk_D64 =
            build_program_from_source(backend_ctx->context, backend_ctx->device, kernel_src.c_str(), compile_opts);

        CL_CHECK((backend_ctx->kernel_mzcache_flash_attn_f16_kvchunk_D64 = clCreateKernel(backend_ctx->program_mzcache_flash_attn_kvchunk_D64, "kernel_mzcache_flash_attn_f16_kvchunk_D64", &err), err));
        GGML_LOG_CONT(".");
    }

    // mzcache : flash_attn_f16_kvchunk
    {
#ifdef GGML_OPENCL_EMBED_KERNELS
        const std::string kernel_src {
    ...
    // mzcache : flash_attn_f16_nolocalKV_D64
    {
#ifdef GGML_OPENCL_EMBED_KERNELS
        const std::string kernel_src {
            #include "mzcache_flash_attn_f16_nolocalKV_D64.cl.h"
        };
#else
        const std::string kernel_src = read_file("mzcache_flash_attn_f16_nolocalKV_D64.cl");
#endif
        backend_ctx->program_mzcache_flash_attn_nolocalKV_D64 =
            build_program_from_source(backend_ctx->context, backend_ctx->device, kernel_src.c_str(), compile_opts);

        CL_CHECK((backend_ctx->kernel_mzcache_flash_attn_f16_nolocalKV_D64 = clCreateKernel(backend_ctx->program_mzcache_flash_attn_nolocalKV_D64, "kernel_mzcache_flash_attn_f16_nolocalKV_D64", &err), err));
        GGML_LOG_CONT(".");
    }

    // mzcache : flash_attn_f16_nolocalKV
    {
#ifdef GGML_OPENCL_EMBED_KERNELS
        const std::string kernel_src {
            #include "mzcache_flash_attn_f16_nolocalKV.cl.h"
        };
#else
        const std::string kernel_src = read_file("mzcache_flash_attn_f16_nolocalKV.cl");
#endif
        backend_ctx->program_mzcache_flash_attn_nolocalKV =
            build_program_from_source(backend_ctx->context, backend_ctx->device, kernel_src.c_str(), compile_opts);

        CL_CHECK((backend_ctx->kernel_mzcache_flash_attn_f16_nolocalKV = clCreateKernel(backend_ctx->program_mzcache_flash_attn_nolocalKV, "kernel_mzcache_flash_attn_f16_nolocalKV", &err), err));
        GGML_LOG_CONT(".");
    }
...
static bool ggml_opencl_supports_op(ggml_backend_dev_t dev, const struct ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_FLASH_ATTN_EXT: // Implemented for mzcache
            if ((op->src[0]->ne[0] == 128 || op->src[0]->ne[0] == 64) && op->src[0]->ne[1] <= 16 && op->src[1]->type == GGML_TYPE_F16) {
                D = op->src[0]->ne[0];
                return true;
            }
        default:
...
struct ggml_backend_opencl_buffer_context {
    ...
    ~ggml_backend_opencl_buffer_context() {
        ...
        if (is_svm) {
            if (svm_base) {
                // clSVMFree(clctx, svm_base); -> This memory will be freed by the mzcache
                svm_base = nullptr;
            }
...
static enum ggml_status ggml_backend_opencl_buffer_init_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor) {
        ...
        if(tensor->is_weight) {
            if (tensor->view_src != nullptr) {
                ...
                if(tensor->view_src->extra){
                    ggml_tensor_extra_cl * view_extra = (ggml_tensor_extra_cl *) tensor->view_src->extra; /// is this how view_src's extra gets passed through?
    ...
    if (tensor->view_src != nullptr) {
        ...
        if(tensor->view_src->extra == nullptr){
            std::cout << "tensor->view_src->extra == nullptr" << "\n";
...
static void ggml_cl_mzcache_flash_attn(ggml_backend_t backend, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst)
{
    ...
    GGML_ASSERT(Q->ne[1] <= NCOLS); // input sequence length
    GGML_ASSERT(Q->ne[1] <= NCOLS); // input sequence length
    GGML_ASSERT(K->ne[1] == V->ne[1]);

    float scale         = 1.0f;   // later from dst->op_params[0]
    ...
    chunk_ptrs * k_svm_chunk_ptrs = (chunk_ptrs *)eK->svm_chunk_ptrs;
    chunk_ptrs * v_svm_chunk_ptrs = (chunk_ptrs *)eV->svm_chunk_ptrs;

    // MZ_LOG_INFO("ggml_cl_mzcache_flash_attn: n_kv_f = %f", ((float *) dst->op_params)[3]);
    ...
    //chunk count and offsets are precomputed; test after implementing the kernel 
    ...
    const cl_ulong offK = eK->offset + K->view_offs;
    const cl_ulong offV = eV->offset + V->view_offs;

    cl_kernel kernel;
    if (D == 64)
        kernel = backend_ctx->kernel_mzcache_flash_attn_f16_nolocalKV_D64;
    else if (D == 128)
        kernel = backend_ctx->kernel_mzcache_flash_attn_f16_nolocalKV;
    else
        GGML_ASSERT(false);

    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_mem),   &eQ->data_device));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_ulong), &offQ));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_mem),   &eK->data_device));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_ulong), &offK));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_mem),   &eV->data_device));
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(cl_ulong), &offV));
...
#else
    CL_CHECK(clSetKernelArg(kernel, arg++, sizeof(int),      &(K->ne[1])));
#endif // MZCACHE
    ...
    const int ntiles_x = ((Q->ne[1] + NCOLS - 1) / NCOLS); // (input_tokens + 16 - 1) / 16 = 1 (input_tokens <= 16)
