...
/*
    ====== How to use ======
    run ./android-build/bin/mz_save_layers_to_files [number of layers to unload] [model path]
    ex) ./mz_save_layers_to_files 28 Qwen3-0.6B-FP16.gguf
*/

// Change the state_name definition to use states directory
static const std::string PROJ_DIR = "/data/local/tmp/hongseung/";
static std::string kv_prefix = PROJ_DIR + "states/";
static std::string weight_prefix = PROJ_DIR + "layers/";

static std::string set_state_name(llama_model* model, common_params& params) {

    std::string state_name = kv_prefix;
...
static std::string set_model_file_name(llama_model* model) {

    std::string model_file_name = model->name;
    std::replace(model_file_name.begin(), model_file_name.end(), ' ', '_');
...
int main(int argc, char** argv) {
    ...
    std::string model_name = argv[2]; // model name
    ...
    params.n_ctx         = 30000;  // Very large context size
    params.n_predict     = 32;   // Maximum number of tokens to generate
    params.n_batch       = 30000;
    params.n_ubatch      = 16;
    params.warmup        = false;
    ...
    std::string layer_file_path = weight_prefix + set_model_file_name(model);
    ...
    MZ_TIME_END(alloc_time);

    std::cout << "========= copy weight =========" << "\n";
    MZ_TIME_START(read_time);
    for(int i = 0; i < number_of_layers_to_test; ++i) {
