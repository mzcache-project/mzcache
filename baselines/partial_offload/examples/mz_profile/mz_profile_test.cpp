...
/*
    ====== How to use ======
    run ./mz_profile_test [number of tensors to unload] [model name][log file path] \
                             [log file name] [mmap usage]
    ex) ./mz_profile_test 50 Qwen3-0.6B-FP16.gguf /data/local/tmp/mskim/experiment test_log false
...
void append_log(const std::string &log_path, const std::string &log_file_name,
    ...
    if (!file_exists) {
        ...
        log_file << "# Format: tensor_num init_mem_bytes unload_mem_bytes load_mem_bytes unload_time_us load_time_us" << std::endl;
        log_file << std::endl;
    }
...
// sample usage: ./android-build/bin/mz_profile_test 308 QWEN /data/local/tmp/mskim/layers log false
int main(int argc, char** argv) {
    ...
    int number_of_tensors_to_test = atoi(argv[1]); // number of test to unload/load 
    ...
    // std::cout << std::fixed << std::setprecision(2);  // fix to 2 decimal places
    // std::cout << "MemAvailable: " << init.mem_available_bytes << " Bytes"
    //           << " (" << to_mb(init.mem_available_bytes) << " MB)" << std::endl;

    size_t n_tensors = llama_model_get_n_tensors(model);
    printf("Total tensors in model: %zu\n", n_tensors);
    profiler.print_model_info();
    std::vector<int> tensors_to_unload = profiler.get_weights();
    // keep only number_of_tensors_to_test tensors, remove the rest
    if (number_of_tensors_to_test < tensors_to_unload.size()) {
        tensors_to_unload.resize(number_of_tensors_to_test);
    }
    ...
    int tensor_num = number_of_tensors_to_test;
    append_log(log_path, log_file_name, tensor_num,
           init_mem_bytes, unload_mem_bytes, load_mem_bytes,
           unload_duration_us, load_duration_us,
           model_name);

    // completion output
    std::cout << "[Log] Tensor " << tensor_num << " recorded." << std::endl;
