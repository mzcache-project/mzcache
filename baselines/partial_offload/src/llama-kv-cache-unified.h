...
public:
    ...
    */
    ...
    /*
      mzCache Minsung Note:
      Free KV tensors of given layers.
    */
    bool state_free(int layers_to_free) override;

    /*
      mzCache Minsung Note:
      Alloc KV tensors of given layers.
    */
    bool state_alloc(int layers_to_free) override;
    ...
    void mzcache_chunk_view_kv() const;
#endif

    // MZCACHE Added for convenience, to avoid using llama_kv_cache_unified_context
   ...
   // mzCache experiemenmtal flag (subject to change)
   bool init_read_done = false;
    /*
      mzCache Minsung Note:
      For partial KV tensor allocation.
      -1 means allocate all(default).
    */
   int n_layers_to_load_for_kv = -1;
   size_t meta_read_cursor = 0;
