...
// mzCache tensor file metadata
struct tensor_file_metadata {
    int tensor_index_in_model;
    size_t offset;
    size_t size;
...
struct llama_model_loader {
    ...
    // mzCache weight tensor metadata map
    std::map<int, tensor_file_metadata> mz_tensor_file_map;
    ...
    /*
      ...
      Load partial tensor data (explicitly used for mzCache).
    */
    bool load_partial_tensor_data(
            struct ggml_context * ctx,
    ...
    /*
      mzCache Minsung Note:
      Load and Copy layers(weights) in the model from file (to SVM).  
    */
    // bool read_and_copy_tensor_data_layerwise_from_files_sync(
    //         struct ggml_context * ctx,
    //         const std::vector<int>& tensors_to_reload,
    //         double& copy_time);

    /*
      mzCache Minsung Note:
      Save layers(weights) in the model to file.
    */
    bool save_weights_in_layers_to_file(
            struct ggml_context * ctx,
