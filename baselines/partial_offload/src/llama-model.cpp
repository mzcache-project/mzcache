...
ggml_context * mz_weight_ctx;
ggml_backend_buffer_type_t mz_weight_buft;
ggml_backend_buffer_t mz_weight_buf;

int layer_0_tensor_idx = -1; // mzCache
int num_tensors_per_layer = -1;

int num_bytes_per_layer = -1; // mzCache
int padded_num_bytes_per_layer = -1; // mzCache

std::vector<size_t> layer_weight_sizes; // mzCache

struct ggml_tensor * layer_first_tensor[MAX_NUM_LAYERS] = {NULL};
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    auto ctx_for_buft = [&](ggml_backend_buffer_type_t buft) -> ggml_context * {
        ...
        // Minsung note: if the context is not found for given buft, create a new one.
        // If the context is found, return the existing one. 
        // (To prevent re-creating duplicate contexts)
        if (it == ctx_map.end()) { // not found
            ggml_init_params params = {
                /*.mem_size   =*/ ctx_size,
                /*.mem_buffer =*/ NULL,
                /*.no_alloc   =*/ true,
    ...
    // create tensors for the weights
    {
        ...
        auto create_tensor = [&](const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne, int flags) -> ggml_tensor * {
            ...
            // std::cout << "create tensor " << tn.str() << "\n";
            // create_tensor_counter++;
            return ml.create_tensor(ctx, tn, ne, flags);
        };

        layers.resize(n_layer);
        std::cout << "LLAMA has " << n_layer << " layers" << "\n";
            ...
            // case LLM_ARCH_GRANITE_MOE:
                {
                    ...
                    push_tensor_name_and_idx(tok_embd, std::string("embed"), -1);
                    // output
                    output_norm = create_tensor(tn(LLM_TENSOR_OUTPUT_NORM, "weight"), {n_embd}, 0);
                    push_tensor_name_and_idx(output_norm, std::string("output"), -1);
                    output      = create_tensor(tn(LLM_TENSOR_OUTPUT,      "weight"), {n_embd, n_vocab}, TENSOR_NOT_REQUIRED);
                    push_tensor_name_and_idx(output, std::string("output"), -1);
                    ...
                    // mzcache: To take out the rope_freqs tensor from the first layer
                    struct ggml_tensor * rope_freqs_once = create_tensor(tn(LLM_TENSOR_ROPE_FREQS, "weight", 0), {n_rot/2}, TENSOR_NOT_REQUIRED);
                    push_tensor_name_and_idx(rope_freqs_once, std::string("rope_freqs"), -1);

                    // HS note : We only need to know the number of tensors per layer.
                    layer_0_tensor_idx = tensor_name_and_idx.size();
                    num_tensors_per_layer = 9;
                    printf("layer_0_tensor_idx = %d\n", layer_0_tensor_idx);


                    layer_weight_sizes.reserve(num_tensors_per_layer);
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        layer.rope_freqs = rope_freqs_once; // use the same tensor for all layers
                        ...
                        layer_first_tensor[i] = layer.attn_norm;
                        push_tensor_name_and_idx(layer.attn_norm, std::string("layer"), i);
                        push_tensor_name_and_idx(layer.wq, std::string("layer"), i);
                        push_tensor_name_and_idx(layer.wk, std::string("layer"), i);
                        push_tensor_name_and_idx(layer.wv, std::string("layer"), i);
                        push_tensor_name_and_idx(layer.wo, std::string("layer"), i);
                        push_tensor_name_and_idx(layer.ffn_norm, std::string("layer"), i);
                        push_tensor_name_and_idx(layer.ffn_gate, std::string("layer"), i);
                        push_tensor_name_and_idx(layer.ffn_down, std::string("layer"), i);
                        push_tensor_name_and_idx(layer.ffn_up, std::string("layer"), i);

                        if(i == 0) {
                            layer_weight_sizes.push_back(ggml_nbytes(layer.attn_norm));
                            layer_weight_sizes.push_back(ggml_nbytes(layer.wq));
                            layer_weight_sizes.push_back(ggml_nbytes(layer.wk));
                            layer_weight_sizes.push_back(ggml_nbytes(layer.wv));
                            layer_weight_sizes.push_back(ggml_nbytes(layer.wo));
                            layer_weight_sizes.push_back(ggml_nbytes(layer.ffn_norm));
                            layer_weight_sizes.push_back(ggml_nbytes(layer.ffn_gate));
                            layer_weight_sizes.push_back(ggml_nbytes(layer.ffn_down));
                            layer_weight_sizes.push_back(ggml_nbytes(layer.ffn_up));

                            num_bytes_per_layer = 0;
                            for (const auto & size : layer_weight_sizes) {
                                num_bytes_per_layer += size;
                            }
                            printf("num_bytes_per_layer = %zu\n", num_bytes_per_layer);
                            padded_num_bytes_per_layer = (num_bytes_per_layer + 4095) / 4096 * 4096; // round up to the nearest 4KiB


                            std::cout << " size of tensor_name_and_idx: " << tensor_name_and_idx.size() << "\n";
                        }
                    }
                } break;
            case LLM_ARCH_EXAONE4:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        // layer.rope_freqs = create_tensor(tn(LLM_TENSOR_ROPE_FREQS, "weight", i), {n_rot/2}, TENSOR_NOT_REQUIRED | (i != 0 ? TENSOR_DUPLICATED : 0));
                        layer.rope_freqs = rope_freqs_once;
                        ...
                        push_tensor_name_and_idx(layer.attn_post_norm, std::string("layer"), i);
                        push_tensor_name_and_idx(layer.attn_q_norm, std::string("layer"), i);
                        push_tensor_name_and_idx(layer.attn_k_norm, std::string("layer"), i);

                        push_tensor_name_and_idx(layer.ffn_gate, std::string("layer"), i);
                        push_tensor_name_and_idx(layer.ffn_down, std::string("layer"), i);
                        push_tensor_name_and_idx(layer.ffn_up, std::string("layer"), i);
                        push_tensor_name_and_idx(layer.ffn_post_norm, std::string("layer"), i);
                        ...
                        if(i == 0) {
                            ...
                            layer_weight_sizes.push_back(ggml_nbytes(layer.attn_post_norm));
                            layer_weight_sizes.push_back(ggml_nbytes(layer.attn_q_norm));
                            layer_weight_sizes.push_back(ggml_nbytes(layer.attn_k_norm));

                            layer_weight_sizes.push_back(ggml_nbytes(layer.ffn_gate));
                            layer_weight_sizes.push_back(ggml_nbytes(layer.ffn_down));
                            layer_weight_sizes.push_back(ggml_nbytes(layer.ffn_up));
                            layer_weight_sizes.push_back(ggml_nbytes(layer.ffn_post_norm));
            ...
            case LLM_ARCH_QWEN3:
                {
                    ...
                    if (output == NULL) {
                        ...
                        // mzCache Minsung note: 
                        // In this case, the output is same as tok_emdb.
                        // So the buffer doesn't exist for output tensor.
                        // *No need to create tensor_name_idx for this case.
                        // push_tensor_name_and_idx(output, std::string("output"));
                    }
    ...
    for (auto & it : ctx_map) {
        ...
        if (ml.use_mmap && use_mmap_buffer && buffer_from_host_ptr_supported && is_default_buft) {
            ...
            // mzcache notes(IMPORTANT): we assume that there are only a single ctx, buft, and buf for each model file. (Except for CPU backend.)
            // So all layers in the model should share the same buf, and we should not create multiple buffers for the same model file.
            // This is important for undefined behavior in mzCache.
            mz_weight_ctx = ctx;
            mz_weight_buft = buft;
            mz_weight_buf = buf;
            ///
...
void llama_model::push_tensor_name_and_idx(ggml_tensor* new_tensor,
                                            std::string name, int layer_index){
    if(new_tensor != NULL || new_tensor != nullptr){
        tensor_name_and_idx.push_back(std::pair<std::string, int>(name, layer_index));
    }
}

/*
    mzCache Minsung Note:
    Returns buft in pimpl of given buf's device type(e.g., CPU or GPU).
    Assumption: the buft is same for all tensors in the same context.
    Might different within different models. (tested only for Qwen3)
*/
ggml_backend_buffer_type_t llama_model::get_buft_from_buf(
                                    ggml_backend_buffer_ptr & buf){
    ggml_backend_buffer_type_t buft = nullptr;
    // Minsung Note:
    // CPU context buffer name : "CPU_Mapped"
    // GPU context buffer name : "OpenCL"
    if (strcmp(ggml_backend_buffer_name(buf.get()), "CPU_Mapped") == 0) {
        if(pimpl->cpu_buft_list.empty() || buf->buft == nullptr){
            throw std::runtime_error(format("llama_model::get_buft_from_buf:\
               cannot find CPU_Mapped buft"));            
            return nullptr;
        }
    }else if(strcmp(ggml_backend_buffer_name(buf.get()), "OpenCL") == 0){
        if(pimpl->gpu_buft_list.empty() || buf->buft == nullptr){
            throw std::runtime_error(format("llama_model::get_buft_from_buf:\
                cannot find OpenCL buft"));            
            return nullptr;
        }
    }
    buft = buf->buft;
...
/*
  mzCache Minsung Note:
  Load tensors(in a single layer) and returns.
  This function seperates 'allocation' from load_buffers_from_file() function for pipelining.
  So, must use with other 'read', 'copy' functions declared in llama-model-loader with threadpool and proper pipelining.
  (see mzCache_loader.cpp for detailed descriptions)
  // Todo:
  // Change to use SVM.
*/
bool llama_model::alloc_buffers_layerwise_sync(int layer_idx){
    ...
    struct ggml_tensor * last_tensor = layer_first_tensor[layer_idx + 1]; // NULL if last layer

    return ggml_backend_alloc_ctx_given_tensors_from_buft(mz_weight_ctx, mz_weight_buf, mz_weight_buft, first_tensor, last_tensor, &n_allocated);
}

// (Final implementation)
static bool read_and_copy_tensor_data_layerwise_from_files_sync(
    const std::string& base_path,
    int layer_idx) {
    // Construct layer-specific file path
    std::string layer_file_path = base_path + "_layer_" + std::to_string(layer_idx) + ".bin";
    int fd = open(layer_file_path.c_str(), O_RDONLY | O_DIRECT);
    if (fd < 0) {
        throw std::runtime_error(
            "Failed to open weight file: " + layer_file_path + ", errno: " + std::strerror(errno)
        );
    }

#ifdef MZCACHE_SVM_KV_CHUNK
    std::cout << "Reading layer " << layer_idx << " from file: " << layer_file_path << std::endl;
    std::cout << "Target memory base: " << layer_first_tensor[layer_idx]->data << std::endl;

    size_t bytes = padded_num_bytes_per_layer;
    if (bytes % 4096 != 0) {
        std::cerr << "Error: bytes (" << bytes << ") is not multiple of 4096\n";
        close(fd);
        return false;
    }

    ssize_t read_bytes = read(fd, layer_first_tensor[layer_idx]->data, padded_num_bytes_per_layer);
    if (read_bytes < 0 || static_cast<size_t>(read_bytes) != padded_num_bytes_per_layer) {
        close(fd);
        std::cout << "read_bytes: " << read_bytes << ", expected size: " << padded_num_bytes_per_layer << std::endl;
        throw std::runtime_error(
    ...
    return true;
#else
    fprintf(stderr, "This function is not implemented for this build.\n");
    close(fd);
    return false;
#endif
}

bool llama_model::copy_weights_to_backend_buffers_layerwise_sync(const std::string& file_path, int layer_idx){
    return read_and_copy_tensor_data_layerwise_from_files_sync(file_path, layer_idx);
}

bool llama_model::unload_buffers(const std::vector<int>& tensors_to_unload) {
    if(pimpl->ctxs.size() < pimpl->bufs.size()) {
        throw std::runtime_error(format("llama_model::unload_buffers:\
            ctxs size %zu is less than bufs size %zu mismatch",
            pimpl->ctxs.size(), pimpl->bufs.size()));
        return false;
    } 
    const int * tensors_to_unload_array = tensors_to_unload.data();
    int size = tensors_to_unload.size();

    ggml_backend_buffer_unload_buffers(mz_weight_buf, tensors_to_unload_array, size);

    std::vector<ggml_tensor *> tensors;

    // save tensor pointers in the context to vector
    ...
    // Unload tensors specified in tensors_to_unload
    for (int i = 0; i < size; ++i) {
        int tensor_idx = tensors_to_unload_array[i];

        if (tensor_idx < 0 || tensor_idx >= (int)tensors.size()) {
            std::cout << ggml_backend_buffer_name(mz_weight_buf) << " unload ";
            std::cout << "llama_model::unload_buffers invalid tensor index " << tensor_idx << "\n";
            continue;
        }
...
/*
  mzCache: load discarded buffers
*/
bool llama_model::load_buffers(llama_model_loader * ml, std::vector<int>& tensors_to_reload,
                                double & weight_alloc_time, double & weight_read_time){
    
    struct timespec alloc_begin, alloc_end;
    struct timespec read_end;
    
    // 1. get ctx, buf to load.
    int buf_idx = 0;
    int n_allocated = 0;
    const auto & use_mlock = params.use_mlock;
    ggml_backend_buffer_type_t buft = nullptr;
    ml->init_mappings(true, use_mlock ? &pimpl->mlock_mmaps : nullptr);
    bool reserve_pimpl_mapping = false;
    if(pimpl->mappings.size() != ml->mappings.size()){
        pimpl->mappings.clear();
        pimpl->mappings.reserve(ml->mappings.size());
        reserve_pimpl_mapping = true;
    }
    for(auto & buf_ptr : pimpl->bufs){
        
        clock_gettime(CLOCK_MONOTONIC, &alloc_begin);
        auto & ctx_ptr = pimpl->ctxs[buf_idx];
        buft = get_buft_from_buf(buf_ptr);
        // 2. use something similar to ggml_backend_alloc_ctx_tensors_from_buft()
        if(!ggml_backend_alloc_ctx_unloaded_tensors_from_buft(ctx_ptr.get(), buf_ptr.get(), buft, &n_allocated)){
            throw std::runtime_error(format("llama_model::load_buffers:\
                failed to allocate ctx tensors from buft %s", ggml_backend_buft_name(buft)));
            return false; 
        }
        clock_gettime(CLOCK_MONOTONIC, &alloc_end);    
        // 3. load all data (probably modified)
        if(n_allocated > 0){
          // mzCache debug load
            // std::cout << "load_partial_tensor_data, load " << n_allocated << " tensors" << "\n";
            // Todo: check reloaded tensor numbers
            if(!ml->load_partial_tensor_data(ctx_ptr.get(), buf_ptr.get(),
              use_mlock ? &pimpl->mlock_mmaps : NULL, params.progress_callback, params.progress_callback_user_data,
              tensors_to_reload)){
                throw std::runtime_error(format("llama_model::load_buffers:\
                    failed to load partial tensor data from buft %s", ggml_backend_buft_name(buft)));
                return false;
            }
        }
        clock_gettime(CLOCK_MONOTONIC, &read_end);
        weight_alloc_time += (alloc_end.tv_sec - alloc_begin.tv_sec) * 1000000 + \
                         (alloc_end.tv_nsec - alloc_begin.tv_nsec) / 1000;     
        weight_read_time += (read_end.tv_sec - alloc_end.tv_sec) * 1000000 + \
                         (read_end.tv_nsec - alloc_end.tv_nsec) / 1000;     
        // else{
        //   std::cout << "no loaded tensors in buffer " << buf_idx << "\n";
        // }
        buf_idx++;   
        n_allocated = 0; 
    }
    if (reserve_pimpl_mapping) {
        for (auto & mapping : ml->mappings) {
            pimpl->mappings.emplace_back(std::move(mapping));
        }
    }
    return true;
    // Todo: need an excepetion catch here.
}

static bool save_weights_in_layers_to_file(
    ...
    constexpr size_t alignment = 4096;  // page size alignment
    size_t total_size = 0;

    // Start from the first tensor of the layer
    ...
    // Write each tensor's data buffer using backend API
    for (int t = 0; t < num_tensors_per_layer; ++t) {
        size_t n_size = ggml_nbytes(cur);
        // Allocate temporary buffer to fetch tensor bytes
        std::vector<uint8_t> tmp_buf(n_size);
        // Use backend API to get tensor data (handles different backends)
    ...
    // Pad the final file size to a 4096-byte boundary
...
bool llama_model::unload_layers_to_file(const std::string& file_path,
                                        int layer_idx){
    if (layer_idx < 0) {
        throw std::runtime_error(format("llama_model::unload_layers_to_file:\
            layer_idx is invalid"));
        return false;
    }
    // create file path with layer index
    std::string layer_file_path = file_path + "_layer_" + std::to_string(layer_idx) + ".bin";

    // if (std::filesystem::exists(layer_file_path)) {
    //     return true;
    // }

    if(!save_weights_in_layers_to_file(mz_weight_ctx, layer_file_path, layer_idx)){
        throw std::runtime_error(format("llama_model::unload_layers_to_file:\
        save_weights_in_layers_to_file returned false"));
        return false;
    }
                    ...
                    if (hparams.swa_type != LLAMA_SWA_TYPE_NONE) {
                        ...
                        res = new llama_kv_cache_unified(
                                *this,
                                ...
                                hparams.n_swa,
                                hparams.swa_type,
                                cparams.n_layers_to_load_for_kv);
                    }
                }
            }
    }
...
llm_graph_result_ptr llama_model::build_graph(
    ...
    switch (arch) {
        ...
        case LLM_ARCH_EXAONE4:
            {
                llm = std::make_unique<llm_build_exaone4>(*this, params, gf);
            } break;
        case LLM_ARCH_RWKV6:
            {
...
/*
  mzCache Minsung Note:
  To add functions here, must add declaration in llama.h
*/
size_t llama_model_get_n_tensors(const llama_model * model) {
    return model->n_tensors();
}

// This function is deprecated
// bool llama_model_unload_buffers(struct llama_model * model, const std::vector<int>& tensors_to_unload) {
//     return model->unload_buffers(tensors_to_unload);
// }
