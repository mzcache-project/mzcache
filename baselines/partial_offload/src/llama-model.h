...
// global weight_context (for mzCache)
...
struct llama_model {
    // mzCache
    friend class mzCache::mzCacheUnloader;
    friend class mzCache::mzCacheLoader;
    ...
    bool unload_buffers(const std::vector<int>& tensors_to_unload); // mzCache
    bool unload_layers_to_file(const std::string & file_path,
                               int layer_idx); // mzCache
    bool alloc_buffers_layerwise_sync(int layer_idx); // mzCache
    bool load_buffers(llama_model_loader * ml, std::vector<int>& tensors_to_reload, 
                      double & weight_alloc_time, double & weight_read_time); // mzCache
    bool copy_weights_to_backend_buffers_layerwise_sync(const std::string& file_path, int layer_idx); // mzCache
    ...
    ggml_backend_buffer_type_t get_buft_from_buf(
                                ggml_backend_buffer_ptr & buf); // mzCache
    ...
    // mzCache 
    // This variable holds weight tensor's 'name','tensor idx' and 'layer num'.
    // 'name' should be one of three.
    // "embed", "layer", "output".
    // the 'tensor idx' is 'idx' of this vector(each std::pair<std::string, int>'s index).
    // the 'layer num' is .second of pair.
    // Layer index -1 means token embedding or output layer.
    std::vector<std::pair<std::string, int>> tensor_name_and_idx;

    // mzCache 
    // This function saves weight tensor's 'name' and 'idx' if given tensor is not nullptr.
    // 'name' should be one of three.
    // "embed", "layer", "output",
    // and the 'tensor idx' is 'idx' of this vector.
    void push_tensor_name_and_idx(ggml_tensor* new_tensor, std::string name, int layer_index);
