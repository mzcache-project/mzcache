...
struct ggml_tensor * llama_model_loader::create_tensor(struct ggml_context * ctx, const std::string & name, const std::initializer_list<int64_t> & ne, int flags) {
    ...
    // std::cout << "Creating tensor: " << name << " with shape: " << llama_format_tensor_shape(ne) << std::endl;
...
void llama_model_loader::init_mappings(bool prefetch, llama_mlocks * mlock_mmaps) {
    /*
      ...
      If mappings already exists, clear
    */
    if (!mappings.empty()){
        mappings.clear();
    }
    // prefetch = false; // mzcache minsung_test
    if (use_mmap) {
...
bool llama_model_loader::load_partial_tensor_data(
        struct ggml_context * ctx,
    ...
    if (upload_backend) {
        LLAMA_LOG_DEBUG("%s: using async uploads for device %s, buffer type %s, backend %s\n", __func__,
            ggml_backend_dev_name(ggml_backend_get_device(upload_backend)),
            ggml_backend_buft_name(ggml_backend_buffer_get_type(buf)),
            ggml_backend_name(upload_backend));
    }
    #ifdef LOAD_LATENCY_DEBUG
        timespec start_mmap, end_mmap;
        timespec start_a, end_a;
        timespec start_b, end_b;
        timespec start_c, end_c;
        clock_gettime(CLOCK_MONOTONIC, &start_mmap);
        long long elapsed_us_a = 0; 
        long long elapsed_us_b = 0; 
        long long elapsed_us_c = 0; 
    #endif
    int tensor_index = 0;
    for (struct ggml_tensor * cur = ggml_get_first_tensor(ctx); cur != NULL; cur = ggml_get_next_tensor(ctx, cur)) {
        if(std::find(tensors_to_reload.begin(), tensors_to_reload.end(), tensor_index) == tensors_to_reload.end()){
            tensor_index++;
            continue;
        }
        tensor_index++;
        ...
        if (use_mmap) {
            ...
            #endif
            const auto & mapping = mappings.at(weight->idx);
            ggml_backend_buffer_t buf_mmap = buf;
            uint8_t * data = (uint8_t *) mapping->addr() + weight->offs;
            ...
            if (check_tensors) {
                ...
                elapsed_us_a += (end_a.tv_sec - start_a.tv_sec) * 1000000LL +
                                (end_a.tv_nsec - start_a.tv_nsec) / 1000LL;
            #endif
            ...
            if (buf_mmap && cur->data == nullptr) {
                ...
                // mzCache vanilla code flow here when mmap==true
                ggml_backend_tensor_set(cur, data, 0, n_size);
            }
            #ifdef LOAD_LATENCY_DEBUG
                clock_gettime(CLOCK_MONOTONIC, &end_b);
                elapsed_us_b += (end_b.tv_sec - start_b.tv_sec) * 1000000LL +
                                (end_b.tv_nsec - start_b.tv_nsec) / 1000LL;
            #endif
        } else {
            const auto & file = files.at(weight->idx);
            if (ggml_backend_buffer_is_host(cur->buffer)) {
                ...
                if (upload_backend) {
                    ...
                    elapsed_us_a += (end_a.tv_sec - start_a.tv_sec) * 1000000LL +
                                    (end_a.tv_nsec - start_a.tv_nsec) / 1000LL;
                    #endif
                } else { // mzCache vanilla code flow here when mmap==false
                    #ifdef LOAD_LATENCY_DEBUG
                    clock_gettime(CLOCK_MONOTONIC, &start_a);
                    #endif
                    read_buf.resize(n_size);
        ...
        long long elapsed_us = (end_mmap.tv_sec - start_mmap.tv_sec) * 1000000LL +
                            (end_mmap.tv_nsec - start_mmap.tv_nsec) / 1000LL;
        std::cout << "model_loader mmap load : "<< elapsed_us <<" us\n";
        std::cout << "model_loader mmap load elapsed_a: "<< elapsed_us_a <<" us\n";
        std::cout << "model_loader mmap load elapsed_b: "<< elapsed_us_b <<" us\n";
    #endif
    // free temporary resources used for async uploads
    for (auto * event : events) {
        ggml_backend_event_synchronize(event);
    ...
    return true;
} // mzcache load_partial_tensor_data()
