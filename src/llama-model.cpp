...
#include "ggml-backend-impl.h" // mzcache: get_buft_from_buf reads ggml_backend_buffer internals
#endif
...
#ifdef MZCACHE_SVM_KV_CHUNK
        // mzcache: only the architectures below are wired up for weight-layer
        // swapout (per-tensor bookkeeping feeds mzcache_globals / mzcache_weight)
        switch (arch) {
            case LLM_ARCH_LLAMA:
                {
                    ...
                    // [mzcache] Shared rope_freqs tensor across all layers
                    ...
                    MZ_LOG_INFO("layer_0_tensor_idx = %d", layer_0_tensor_idx);
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        if(i == 0) {
                            ...
                            MZ_LOG_INFO("num_bytes_per_layer = %zu", num_bytes_per_layer);
                            padded_num_bytes_per_layer = (num_bytes_per_layer + 4095) / 4096 * 4096; // round up to the nearest 4KiB


                            MZ_LOG_INFO("tensor_name_and_idx size = %zu", tensor_name_and_idx.size());
                        }
                    }
                } break;
            case LLM_ARCH_EXAONE4:
            ...
            case LLM_ARCH_QWEN3:
                {
                    ...
                    if (output == NULL) {
                        ...
                        // [mzcache] output == tok_embd (shared), no separate tensor_name_idx entry
                    }

                    // [mzcache] Track per-layer tensor layout
                    layer_0_tensor_idx = tensor_name_and_idx.size();
                    MZ_LOG_INFO("layer_0_tensor_idx = %d", layer_0_tensor_idx);
                    num_tensors_per_layer = 11;
...
#ifdef MZ_MLOCK_TOKEN_EMBD
    // exp1_android_swap baseline: pin the token embedding in RAM (self-populates
    // mlock_mmaps regardless of use_mlock; requires use_mmap).
    ml.mlock_tensor_range_only("token_embd.weight", &pimpl->mlock_mmaps);
#endif
...
#ifdef MZCACHE_SVM_KV_CHUNK
            // [mzcache] Capture the single weight ctx/buft/buf (assumes one GPU buffer for all layers)
            mz_weight_ctx  = ctx;
            mz_weight_buft = buft;
            mz_weight_buf  = buf;
#endif
...
// mzcache: returns the buft of the given buffer (CPU_Mapped or OpenCL).
// Assumes the buft is the same for all tensors in the same context.
...
// ---- [mzcache] begin: thin wrappers delegating to mzcache/src/mzcache_weight_ops.cpp ----
#include "mzcache_weight_ops.h"

bool llama_model::alloc_weight_layer(int layer_idx) {
    return mzcache::alloc_weight_layer(layer_idx);
}

bool llama_model::read_weight_layer(int & fd, int layer_idx) {
    return mzcache::read_weight_layer(fd, layer_idx);
}

bool llama_model::unload_buffers(int layer_idx) {
    return mzcache::unload_buffers(layer_idx);
}

bool llama_model::unload_layers_to_file(const std::string & file_path, int layer_idx) {
    return mzcache::unload_layers_to_file(file_path, layer_idx);
}
// ---- [mzcache] end ----
#endif // MZCACHE_SVM_KV_CHUNK
