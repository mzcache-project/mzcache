...
struct llama_model {
#ifdef MZCACHE_SVM_KV_CHUNK
    friend class mzcache::mzcacheUnloader;  // [mzcache]
    friend class mzcache::mzcacheLoader;    // [mzcache]
#endif
...
#ifdef MZCACHE_SVM_KV_CHUNK
    // ---- [mzcache] begin: weight layer management (delegates to mzcache/src/mzcache_weight_ops.cpp) ----
...
#ifdef MZCACHE_SVM_KV_CHUNK
    // [mzcache] Per-tensor metadata: vector index = tensor index, pair = (category, layer).
    // category: "embed", "layer", "output". layer = -1 for non-layer tensors.
