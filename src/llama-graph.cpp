...
#ifdef MZCACHE_SVM_KV_CHUNK
    // [mzcache] chunked K/V tensors hide the true KV length, so it is passed in
    const auto ne11     = n_kv > 0 ? n_kv : k->ne[1];
#else
    const auto ne11     = k->ne[1];
#endif

    ggml_tensor * cur;

    // TODO: replace hardcoded padding with ggml-provided padding
    if (cparams.flash_attn && (ne11 % 256 == 0) && kq_b == nullptr) {
        ...
        // [mzcache] SVM chunk mode passes ne11 to size output tensor correctly
#ifndef MZCACHE_SVM_KV_CHUNK
        cur = ggml_flash_attn_ext(ctx0, q, k, v, kq_mask, kq_scale, hparams.f_max_alibi_bias,
                                  hparams.attn_soft_cap ? hparams.f_attn_logit_softcapping : 0.0f);
#else
        cur = ggml_flash_attn_ext(ctx0, q, k, v, kq_mask, kq_scale, hparams.f_max_alibi_bias,
                                  hparams.attn_soft_cap ? hparams.f_attn_logit_softcapping : 0.0f, ne11);
#endif
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    // [mzcache] SVM chunk mode passes n_kv for chunk-aware tensor sizing
#ifndef MZCACHE_SVM_KV_CHUNK
    ggml_tensor * cur = build_attn_mha(gf, q, k, v, kq_b, kq_mask, v_mla, kq_scale);
#else
    const auto n_kv = mctx_cur->get_n_kv();
    ggml_tensor * cur = build_attn_mha(gf, q, k, v, kq_b, kq_mask, v_mla, kq_scale, n_kv);
#endif
    cb(cur, "kqv_out", il);
