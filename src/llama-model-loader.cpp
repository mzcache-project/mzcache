...
// exp1_android_swap baseline: mlock only the page range backing a single named
// tensor (token_embd.weight), so the OS cannot swap the embedding out while the
// rest of the model pages naturally. Ported from the baseline-test branch.
bool llama_model_loader::mlock_tensor_range_only(const char * tensor_name, llama_mlocks * mlock_mmaps) {
    if (!llama_mlock::SUPPORTED) {
        LLAMA_LOG_WARN("%s: mlock is not supported on this platform\n", __func__);
        return false;
    }
    if (!use_mmap) {
        LLAMA_LOG_WARN("%s: mlock requires mmap to be enabled\n", __func__);
        return false;
    }
    if (!mlock_mmaps) {
        LLAMA_LOG_WARN("%s: mlock_mmaps is null\n", __func__);
        return false;
    }

    const llama_tensor_weight * weight = get_weight(tensor_name);
    if (!weight) {
    ...
    const size_t tensor_offs = weight->offs;
    const int    weight_idx  = weight->idx;

    if (mlock_mmaps->size() <= (size_t) weight_idx) {
        mlock_mmaps->resize(weight_idx + 1);
    }
    if (!mlock_mmaps->at(weight_idx)) {
        mlock_mmaps->at(weight_idx) = std::make_unique<llama_mlock>();
        const auto & mapping = mappings.at(weight_idx);
        mlock_mmaps->at(weight_idx)->init(mapping->addr());
    }
...
#endif
    const size_t start_offset = tensor_offs & ~(page_size - 1);
    const size_t end_offset   = (tensor_offs + tensor_size + page_size - 1) & ~(page_size - 1);
    const size_t aligned_size = end_offset - start_offset;

    try {
        mlock_mmaps->at(weight_idx)->grow_to(aligned_size);
        LLAMA_LOG_INFO("%s: locked tensor '%s' (%zu bytes at offset %zu)\n",
                       __func__, tensor_name, tensor_size, tensor_offs);
        return true;
    } catch (const std::exception & e) {
        LLAMA_LOG_WARN("%s: failed to lock tensor '%s': %s\n", __func__, tensor_name, e.what());
        return false;
    }
}
#endif // MZ_MLOCK_TOKEN_EMBD

void llama_model_loader::init_mappings(bool prefetch, llama_mlocks * mlock_mmaps) {
#ifdef MZCACHE_SVM_KV_CHUNK
    // [mzcache] the loader can be re-initialized; drop stale mappings
    if (!mappings.empty()){
        mappings.clear();
    }
#endif
