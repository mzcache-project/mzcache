...
#include "mzcache_types.h" // [mzcache] ggml_tensor_ptrs (per-layer chunk tables)
#endif
...
public:
    ...
    // Number of chunk slots materialized per tensor. Only chunk 0 exists after
    // construction; mz_grow_chunks() extends every layer in lockstep as the
    // cache fills (state restore, decode past a 256-token boundary).
    uint32_t n_chunks_alloc = 0;

    // Ensure chunks covering the first `n_tokens_needed` cells exist in all
    // layers, allocating and publishing any missing ones. No-op if already
    // covered. Notifies g_mzcache_core (if any) so mzcache bookkeeping follows.
    void mz_grow_chunks(uint32_t n_tokens_needed);
