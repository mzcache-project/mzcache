...
#include "mzcache_core.h" // g_mzcache_core: bookkeeping follows chunk growth
#endif
...
llama_kv_cache_unified::llama_kv_cache_unified(
...
#else
        // [mzcache] Chunked KV tensor creation (base chunk only; the rest are
        // materialized on demand by mz_grow_chunks as the cache fills)
        mzcache::create_chunked_kv_tensors(ctx, type_k, type_v,
            n_embd_k_gqa, n_embd_v_gqa, kv_size, il, k, v,
            layers_k_chunks, layers_v_chunks);
    ...
    // [mzcache] Register SVM chunk offsets
#ifdef MZCACHE_SVM_KV_CHUNK
    if (model.dev_layer(0) != model.dev_layer(hparams.n_layer - 1)) {
        MZ_LOG_ERROR("MZCACHE requires all layers on the same device, but found different devices for layers 0 and %d",
                hparams.n_layer - 1);
        exit(1);
    }
    mzcache::register_kv_chunk_offsets(hparams.n_layer, kv_size,
        layers_k_chunks, layers_v_chunks, bufs);
#endif
...
#ifdef MZCACHE_SVM_KV_CHUNK
void llama_kv_cache_unified::mz_grow_chunks(uint32_t n_tokens_needed) {
    const uint32_t need = (n_tokens_needed + TOKENS_PER_CHUNK - 1) / TOKENS_PER_CHUNK;
    if (need <= n_chunks_alloc) {
        return;
    }
    if (need > MAX_CHUNKS_PER_TENSOR) {
        throw std::runtime_error("mz_grow_chunks: token count exceeds MAX_CHUNKS_PER_TENSOR");
    }

    ggml_context * ctx = ctxs.front().get(); // single buft: all layers share one context

    for (const auto & layer : layers) {
        const uint32_t il = layer.il;
        for (uint32_t idx = n_chunks_alloc; idx < need; ++idx) {
            mzcache::grow_layer_chunk(ctx, il, idx, hparams.n_embd_k_gqa(il),
                                      layers_k_chunks, layers_v_chunks);
        }
    }

    LLAMA_LOG_INFO("%s: KV chunks grown %u -> %u (covers %u tokens)\n",
                   __func__, n_chunks_alloc, need, need * TOKENS_PER_CHUNK);
    n_chunks_alloc = need;

    // Let mzcache's swapout/swapin bookkeeping follow (raw chunk pointers,
    // chunk states, counters). No-op before a core exists.
    if (g_mzcache_core) {
        g_mzcache_core->on_kv_chunks_grown((int) need);
    }
}
...
#ifdef MZCACHE_SVM_KV_CHUNK
    // [mzcache] The write below may land in a chunk that has not been
    // materialized yet (decode past a 256-token boundary): grow first.
    mz_grow_chunks(head_cur + ubatch.n_tokens);
#endif
...
#else
    // [mzcache] Copy K cache using chunk-local offset
    last_chunk_idx = head_cur / TOKENS_PER_CHUNK;
    int chunk_offset = head_cur % TOKENS_PER_CHUNK;

    if (chunk_offset + n_tokens > TOKENS_PER_CHUNK) {
        LLAMA_LOG_ERROR("%s: chunk overflow: offset %d + n_tokens %d > %d\n",
                        __func__, chunk_offset, (int)n_tokens, TOKENS_PER_CHUNK);
        return nullptr;
    }
...
#else
        // [mzcache] Copy V cache using chunk-local offset
        last_chunk_idx = head_cur / TOKENS_PER_CHUNK;
        int chunk_offset = head_cur % TOKENS_PER_CHUNK;

        if (chunk_offset + n_tokens > TOKENS_PER_CHUNK) {
            LLAMA_LOG_ERROR("%s: chunk overflow: offset %d + n_tokens %ld > %d\n",
...
void llama_kv_cache_unified::state_write_data(llama_io_write_i & io, const std::vector<std::pair<uint32_t, uint32_t>> & cell_ranges) const {
    ...
    for (const auto & layer : layers) {
        ...
        // key row size (one token = one row)
...
#else
        mzcache::state_write_k_chunked(io, il, k_size_row, cell_ranges); // [mzcache]
#endif
    }

    // --- Write V ---
    if (!v_trans) {
        // non-transposed V: row = one token
        for (const auto & layer : layers) {
            const uint32_t il = layer.il;
            const uint32_t n_embd_v_gqa = hparams.n_embd_v_gqa(il);
            ...
            // value row size (one token = one row)
...
#else
            mzcache::state_write_v_chunked(io, il, v_size_row, cell_ranges); // [mzcache]
#endif
        }
    } else {
        // transposed V: element-wise ranges, as upstream
        ...
        for (const auto & layer : layers) {
            ...
            for (uint32_t j = 0; j < n_embd_v_gqa; ++j) {
                for (const auto & range : cell_ranges) {
                    const size_t range_size = range.second - range.first;
                    const size_t src_offset = (range.first + j * (size_t)kv_size) * v_size_el;
...
bool llama_kv_cache_unified::state_read_data(llama_io_read_i & io, uint32_t cell_count) {
    ...
    // [mzcache] Materialize the chunks the restored range lands in.
    mz_grow_chunks(head + cell_count);
#endif
...
#else
            mzcache::state_read_k_chunked(io, il, k_size_row, cell_count, head, layers_k_chunks); // [mzcache]
#endif
        }
    }
...
#else
                mzcache::state_read_v_chunked(io, il, v_size_row, cell_count, head, layers_v_chunks); // [mzcache]
#endif
            }
        }
    } else {
